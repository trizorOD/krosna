...
if ( ! class_exists( 'Learts_Helper' ) ) {
	...
	class Learts_Helper {
		...
		public function __construct() {
			add_action( 'wp_ajax_learts_ajax_search', array( $this, 'ajax_search' ) );
			add_action( 'wp_ajax_nopriv_learts_ajax_search', array( $this, 'ajax_search' ) );
			add_filter( 'posts_where', array( $this, 'title_like_posts_where' ), 10, 2 );

			add_action( 'wp_ajax_learts_ajax_load_more', array( $this, 'ajax_load_more' ) );
			add_action( 'wp_ajax_nopriv_learts_ajax_load_more', array( $this, 'ajax_load_more' ) );
		}
		...
		public static function get_all_menus( $allow_empty = false, $vc = false ) {
			...
			if ( $allow_empty ) {
				$menus = array( esc_html__( '-- Select a menu --', 'learts' ) => '' );
			}
		...
		public static function get_registered_sidebars( $default_option = false, $disable_option = false ) {
			...
			if ( $default_option ) {
				$sidebars['default'] = esc_html__( 'Default', 'learts' );
			}

			if ( $disable_option ) {
				$sidebars['no'] = esc_html__( 'Disable', 'learts' );
			}
		...
		public static function get_list_page_layout() {
			return array(
				'fullwidth'       => LEARTS_FRAMEWORK_URI . '/assets/images/1c.png',
				'content-sidebar' => LEARTS_FRAMEWORK_URI . '/assets/images/2cr.png',
				'sidebar-content' => LEARTS_FRAMEWORK_URI . '/assets/images/2cl.png',
			);
		}
		...
		public static function get_config( $setting = '' ) {
			...
			$path = LEARTS_THEME_DIR . DS . 'assets' . DS . 'json' . DS . 'config.json';
		...
		public static function social_icons( $custom = true, $colors = false ) {
			...
			$networks = array(
				...
				'envelope-o'     => array(
					'label' => esc_html__( 'Email Address', 'learts' ),
					'color' => '#000000',
				),
			...
			if ( $custom ) {
				$networks['custom'] = array( 'label' => esc_attr__( 'Custom', 'learts' ), 'color' => '' );
			}
		...
		public static function get_the_title() {
			...
			if ( is_front_page() && ! is_singular( 'page' ) ) {
				$title = esc_html__( 'Our Blog', 'learts' );
			...
			if ( is_search() ) {
				...
				$title = apply_filters( 'learts_page_title',
					'<span id="search-results-count">' . $wp_query->found_posts . '</span> ' . _n( 'Результати пошуку',
			...
			if ( is_tag() || is_category() || is_date() || is_author() ) {
				...
				if ( is_author() ) {
					...
					// Daily archive title.
					$title = sprintf( esc_html__( 'Daily Archives: %s', 'learts' ), get_the_date() );
				} elseif ( is_month() ) {
					// Monthly archive title.
					$title = sprintf( esc_html__( 'Monthly Archives: %s', 'learts' ),
						get_the_date( esc_html_x( 'F Y', 'Page title monthly archives date format', 'learts' ) ) );
				} elseif ( is_year() ) {
					// Yearly archive title.
					$title = sprintf( esc_html__( 'Yearly Archives: %s', 'learts' ),
						get_the_date( esc_html_x( 'Y', 'Page title yearly archives date format', 'learts' ) ) );
				} else {
			...
			if ( is_404() ) {
				...
				$title = esc_html__( '404: Page Not Found', 'learts' );
			...
			if ( $post_id ) {
				...
				if ( is_singular( 'post' ) ) {
					$title = get_post_meta( $post_id,
						'learts_post_title_on_top',
						true ) ? get_the_title( $post_id ) : esc_html__( 'Our Blog', 'learts' );
				} elseif ( is_singular( 'product' ) ) {
					$title = get_post_meta( $post_id,
						'learts_product_title_on_top',
				...
				// Custom meta title.
				if ( $custom_title = get_post_meta( $post_id, 'learts_custom_page_title', true ) ) {
					$title = $custom_title;
				}
			}
			...
			if ( class_exists( 'WooCommerce' ) && ( is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() ) ) {
				...
				if ( apply_filters( 'woocommerce_show_page_title', true ) && ! is_product() ) {
					...
					if ( is_shop() && $custom_title = get_post_meta( $post_id, 'learts_custom_page_title', true ) ) {
						$title = $custom_title;
					}
			...
			return apply_filters( 'learts_page_title', $title );;
		}
		...
		public function ajax_search() {
			...
			if ( ! empty( $_REQUEST['query'] ) ) {
				...
				$post_type      = isset( $_REQUEST['post_type'] ) ? $_REQUEST['post_type'] : learts_get_option( 'search_post_type' );
				$posts_per_page = isset( $_REQUEST['limit'] ) ? $_REQUEST['limit'] : learts_get_option( 'search_limit' );
				...
				$thumb_size     = apply_filters( 'learts_search_thumb_size', 'learts-search-thumb' );
						...
						if ( ! empty( $search_ids ) ) {
							...
							$suggestions = array(
								'suggestions' => array(
									array(
										'id'    => - 1,
										'value' => esc_html__( 'Здається, ми не можемо знайти те, що ви шукаєте. Можливо, пошук допоможе.',
											'learts' ),
										'url'   => '',
									),
								),
				...
				if ( $posts->have_posts() ) {
					...
					if ( intval( $posts->found_posts ) > $posts_per_page ) {
						...
						$suggestions[] = array(
							'id'     => - 2,
							'value'  => esc_html__( 'View All', 'learts' ),
							'url'    => esc_url( home_url( '/' ) ) . $query_str,
							'target' => apply_filters( 'learts_search_view_all_target', '_blank' ),
						);
					}
		...
		public function ajax_load_more() {
			...
			if ( $post_type == 'post' ) {
				...
				$shortcode_post_class = isset( $_POST['columns'] ) ? 'col-lg-' . learts_VC::calculate_column_width( $_POST['columns'] ) : 'col-lg-3';
			...
			if ( $post_type == 'product' ) {
				...
				$query = Learts_Woo::get_products_by_datasource( $data_source, $atts, $args );

				add_filter( 'learts_shop_products_columns',
					function () {
				...
				add_filter( 'learts_shop_products_classes',
					function () {
						return 'adding-item';
					} );
			}
			...
			if ( $query->have_posts() ) {
				...
				while ( $query->have_posts() ) {
					...
					if ( $post_type == 'post' ) {
						...
						add_filter( 'learts_blog_image_size',
							function () {
								return ( $view == 'grid' ) ? 'learts-post-grid' : 'learts-single-thumb';
							} );
					...
					<?php if ( $post_type == 'portfolio' ) {
						set_query_var('set_columns_portfolio', $_REQUEST['columns']);
						include( locate_template( 'components/portfolio/content-portfolio.php' ) );
					}
				}
		...
		public static function get_active_sidebar( $shop = false ) {
			...
			$page_id       = Learts_Helper::get_the_ID();

			// DEFAULT SIDEBAR CONFIG - Page.
			$default_sidebar_config = learts_get_option( 'page_sidebar_config' );
			$sidebar                = get_post_meta( $page_id, 'learts_page_custom_sidebar', true );

			// DEFAULT SIDEBAR CONFIG - Blog.
			if ( is_archive() || is_author() || is_category() || is_home() ) {
				$default_sidebar_config = learts_get_option( 'archive_sidebar_config' );
				$sidebar                = learts_get_option( 'archive_sidebar' );
			}

			// For search page
			if ( is_search() ) {
				$default_sidebar_config = learts_get_option( 'search_sidebar_config' );
				$sidebar                = learts_get_option( 'search_sidebar' );
			}

			// DEFAULT SIDEBAR CONFIG - Single Post.
			if ( is_singular( 'post' ) ) {
				$sidebar_config = get_post_meta( $page_id, 'learts_post_sidebar_config', true );
				$sidebar        = get_post_meta( $page_id, 'learts_post_custom_sidebar', true );
			}
			...
			if ( $shop ) {
				...
				// shop page
				if ( class_exists( 'WooCommerce' ) && ( is_shop() || is_product_taxonomy() ) && ! is_singular( 'product' ) && ! is_search() ) {
					$sidebar_config = learts_get_option( 'shop_sidebar_config' );
					$sidebar        = learts_get_option( 'shop_sidebar' );
				}

				// In single product page
				if ( is_product() ) {
					$sidebar_config = get_post_meta( $page_id, 'learts_product_sidebar_config', true );
					$sidebar        = get_post_meta( $page_id, 'learts_product_custom_sidebar', true );

					if ( $sidebar_config == 'default' ) {
						$sidebar_config = learts_get_option( 'product_sidebar_config' );
						$sidebar        = learts_get_option( 'product_sidebar' );
					}
				}
		 ...
		 * @return mixed|void
		 */
		public static function get_widget_column_class( $sidebar_id = '' ) {
		...
		public static function get_rev_sliders() {
			global $wpdb;
			$revsliders = array(
				'' => esc_html__( 'Choose a slider', 'learts' ),
			);
		...
		public static function slider() {
			...
			$slider = get_post_meta( $id, 'revolution_slider', true );
		...
		public static function wp_kses_allowed_html( $context ) {
