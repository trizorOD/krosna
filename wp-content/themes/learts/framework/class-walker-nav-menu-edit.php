...
if ( ! class_exists( 'learts_Walker_Nav_Menu_Edit' ) ) {
	/**
	 ...
	 */
	class learts_Walker_Nav_Menu_Edit extends Walker_Nav_Menu {
		...
		public function setup_admin_scripts() {
			...
			wp_enqueue_script( 'rgba-picker-js',
				LEARTS_THEME_URI . '/assets/admin/js/rgba-picker.min.js',
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( ! empty( $item->_invalid ) ) {
				...
				$title = sprintf( __( '%s (Invalid)', 'learts' ), $item->title );
			} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				$classes[] = 'pending';
				/* translators: %s: title of menu item in draft status */
				$title = sprintf( __( '%s (Pending)', 'learts' ), $item->title );
			}
					...
					<span class="item-controls">
                        ...
                        <span class="item-order hide-if-js">
                            ...
                            echo wp_nonce_url( add_query_arg( array(
	                            ...
	                            'move-menu_item' );
                            ?>" class="item-move-up"
                               aria-label="<?php esc_attr_e( 'Move up', 'learts' ) ?>">&#8593;</a>
                            |
                            <a href="<?php
                            echo wp_nonce_url( add_query_arg( array(
	                            ...
	                            remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ),
	                            'move-menu_item' );
                            ?>" class="item-move-down" aria-label="<?php esc_attr_e( 'Move down', 'learts' ) ?>">&#8595;</a>
                        </span>
                        ...
                        ?>"
                           aria-label="<?php esc_attr_e( 'Edit menu item', 'learts' ); ?>"><?php esc_html_e( 'Edit',
		                        'learts' ); ?></a>
                    </span>
				</div>
			</div>
			...
			<div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
				...
				<p class="description description-wide">
					<label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Navigation Label', 'learts' ); ?><br/>
				...
				<p class="field-title-attribute field-attr-title description description-wide">
					<label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Title Attribute', 'learts' ); ?><br/>
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
						<input type="checkbox" id="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>"
							...
							'_blank' ); ?> />
						<?php esc_html_e( 'Open link in a new tab', 'learts' ); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'CSS Classes (optional)', 'learts' ); ?><br/>
				...
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Link Relationship (XFN)', 'learts' ); ?><br/>
				...
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Description', 'learts' ); ?><br/>
				...
				<p class="field-move hide-if-no-js description description-wide">
					<label>
						<span><?php esc_html_e( 'Move', 'learts' ); ?></span>
						<a href="#" class="menus-move menus-move-up"
						   data-dir="up"><?php esc_html_e( 'Up one', 'learts' ); ?></a>
						<a href="#" class="menus-move menus-move-down"
						   data-dir="down"><?php esc_html_e( 'Down one', 'learts' ); ?></a>
						<a href="#" class="menus-move menus-move-left" data-dir="left"></a>
						<a href="#" class="menus-move menus-move-right" data-dir="right"></a>
						<a href="#" class="menus-move menus-move-top"
						   data-dir="top"><?php esc_html_e( 'To the top', 'learts' ); ?></a>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if ( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( __( 'Original: %s', 'learts' ),
					...
					<a class="item-delete submitdelete deletion" id="delete-<?php echo esc_attr( $item_id ); ?>"
					   ...
					   echo wp_nonce_url( add_query_arg( array(
						   ...
						   remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ),
						   'delete-menu_item_' . $item_id ); ?>"><?php esc_html_e( 'Remove', 'learts' ); ?></a>
	...
	// add custom field
	add_action( 'wp_nav_menu_item_custom_fields', 'learts_nav_menu_item_custom_fields', 9, 4 );
	function learts_nav_menu_item_custom_fields( $item_id, $item, $depth, $args ) { ?>
		...
		<p class="description description-wide custom-colors"
		   style="margin-top:10px;<?php echo( 'custom' != $item->tag_type ? 'display:none;' : '' ); ?>">
			<strong><?php esc_html_e( 'Normal State Colors', 'learts' ) ?></strong></p>

		<p class="description description-wide custom-colors"<?php echo( 'custom' != $item->tag_type ? ' style="display:none;"' : '' ); ?>>
			<span class="display:block;"><?php esc_html_e( 'Text Color: ', 'learts' ) ?></span>
		...
		<p class="description description-wide custom-colors"<?php echo( 'custom' != $item->tag_type ? ' style="display:none;"' : '' ); ?>>
			<span class="display:block;"><?php esc_html_e( 'Background Color: ', 'learts' ) ?></span>
		...
		<p class="description description-wide custom-colors"<?php echo( 'custom' != $item->tag_type ? ' style="display:none;"' : '' ); ?>
		   style="margin-top:10px;<?php 'custom' != $item->tag_type ? 'display:none;' : ''; ?>">
			<strong><?php esc_html_e( 'Hover State Colors', 'learts' ) ?></strong></p>
		...
		<p class="field-tag description description-wide"
		   style="margin-bottom: 10px;<?php echo( 'custom' != $item->tag_type ? ' display:none;' : '' ); ?>">
			<label for="menu_item_tag-<?php echo esc_attr( $item_id ); ?>">
				<?php esc_html_e( 'Tag Text', 'learts' ); ?><br/>
		...
		<p class="field-icon-classes description description-wide">
			<label for="menu_item_icon_classes-<?php echo esc_attr( $item_id ); ?>">
				<?php esc_html_e( 'Icon Classes (optional)', 'learts' ); ?><br/>
				<input type="text"
		...
		<p class="description description-wide">
			<label for="menu_item_layout-<?php echo esc_attr( $item_id ); ?>">
				<?php esc_html_e( 'Mega Menu Layout', 'learts' ); ?><br>
				<select id="menu_item_layout-<?php echo esc_attr( $item_id ); ?>" class="widefat"
					...
					<option
						...
						'default',
						true ); ?>><?php esc_html_e( 'Default', 'learts' ); ?></option>
					<option
						value="wide" <?php selected( $item->layout,
						'wide',
						true ); ?>><?php esc_html_e( 'Wide', 'learts' ); ?></option>
					<option
						value="full-width" <?php selected( $item->layout,
						'full-width',
						true ); ?>><?php esc_html_e( 'Full width', 'learts' ); ?></option>
					<option
						value="custom" <?php selected( $item->layout, 'custom', true ); ?>><?php esc_html_e( 'Custom',
							'learts' ); ?></option>
				</select>
			</label>
			<span class="description"><?php esc_html_e( 'Select the mega menu layout', 'learts' ); ?></span>
		</p>
		<p class="description description-thin custom-width"<?php echo( 'custom' != $item->layout ? ' style="display:none;"' : '' ); ?>>
			<label for="menu_item_width-<?php echo esc_attr( $item_id ); ?>">
				<?php esc_html_e( 'Mega Menu width', 'learts' ); ?><br>
	 ...
	 */
	add_action( 'wp_update_nav_menu_item', 'learts_nav_update', 10, 3 );
	function learts_nav_update( $menu_id, $menu_item_db_id, $args ) {
	...
	/*
	 * Adds value of new field to $item object that will be passed to     learts_Walker_Nav_Menu_Edit
	 */
	add_filter( 'wp_setup_nav_menu_item', 'learts_nav_item' );
	function learts_nav_item( $menu_item ) {
	...
	add_filter( 'wp_edit_nav_menu_walker', 'learts_nav_edit_walker', 10, 2 );
	function learts_nav_edit_walker( $walker, $menu_id ) {
		return 'learts_Walker_Nav_Menu_Edit';
	}
}
