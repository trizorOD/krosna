 ...
 */
if ( ! class_exists( 'Learts_Instagram' ) ) {

	class Learts_Instagram {
		...
		public static function scrape_instagram( $username, $number, $square = true ) {
			...
			if ( false === ( $instagram = get_transient( 'instagram-media-2' . sanitize_title_with_dashes( $username . '-' . $square ) ) ) ) {
				...
				if ( is_wp_error( $remote ) ) {
					return new WP_Error( 'site_down', esc_html__( 'Unable to communicate with Instagram.', 'learts' ) );
				}

				if ( 200 != wp_remote_retrieve_response_code( $remote ) ) {
					return new WP_Error( 'invalid_response', esc_html__( 'Instagram did not return a 200.', 'learts' ) );
				}
				...
				if ( ! $insta_arr ) {
					return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned invalid data.', 'learts' ) );
				}
				...
				if ( isset( $insta_arr['entry_data']['ProfilePage'][0]['graphql']['user']['edge_owner_to_timeline_media']['edges'] ) ) {
					...
					return new WP_Error( 'bad_json_2', esc_html__( 'Instagram has returned invalid data.', 'learts' ) );
				}

				if ( ! is_array( $media_arr ) ) {
					return new WP_Error( 'bad_array', esc_html__( 'Instagram has returned invalid data.', 'learts' ) );
				}
			...
			if ( ! empty( $instagram ) ) {
				...
				return new WP_Error( 'no_images', esc_html__( 'Instagram did not return any images.', 'learts' ) );
					...
					if ( ! empty( $user_remote['body'] ) ) {
						...
						if ( ! $user_arr ) {
							return new WP_Error( 'bad_json',
								esc_html__( 'Instagram has returned invalid data.', 'learts' ) );
						}
