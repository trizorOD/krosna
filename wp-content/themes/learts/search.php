...
global $learts_has_blog;
$learts_has_blog = true;

// Variables.
$page_wrap_class = $content_class = '';

// Sidebar config.
$search_sidebar_config = learts_get_option( 'search_sidebar_config' );

if ( $search_sidebar_config == 'left' ) {
	$page_wrap_class = 'has-sidebar-left row';
	$content_class   = 'col-xs-12 col-md-8 col-lg-9';
} else if ( $search_sidebar_config == 'right' ) {
	...
	<div class="container">
		<div class="inner-page-wrap <?php echo esc_attr( $page_wrap_class ); ?>">
			<div id="main" class="site-content <?php echo esc_attr( $content_class ); ?>" role="main">
				<?php do_action( 'learts_base_loop' ); ?>
			</div>
