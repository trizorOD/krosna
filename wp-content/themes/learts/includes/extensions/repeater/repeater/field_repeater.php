	...
	class ReduxFramework_repeater {
		...
		public function __construct( $field = array(), $value = '', $parent ) {
			...
			if ( empty( $this->extension_dir ) ) {
				...
				$this->extension_url = trailingslashit( get_template_directory_uri() ) . 'includes/extensions/repeater/repeater/';
		...
		public function render() {
			...
			if ( isset ( $this->value['Redux_repeater_data'] ) && is_array( $this->value['Redux_repeater_data'] ) && ! empty ( $this->value['Redux_repeater_data'] ) ) {
				...
				foreach ( $repeaters as $repeater ) {
					...
					if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
						echo '<a href="javascript:void(0);" class="button deletion redux-repeaters-remove">' . __( 'Delete', 'learts' ) . ' ' . $this->field['item_name'] . '</a>';
					}
					echo '</div>';
					echo '</fieldset>';
					echo '</table>';
			...
			if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
				...
				echo '<a href="javascript:void(0);" class="button redux-repeaters-add button-primary' . $disabled . '" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->parent->args['opt_name'] . $this->repeater_values . '[title][]">' . __( 'Add', 'learts' ) . ' ' . $this->field['item_name'] . '</a><br/>';
			}
