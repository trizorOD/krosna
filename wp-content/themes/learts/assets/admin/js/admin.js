...
(function( $ ) {
	...
	leartsAdmin = (
		function() {
			...
			var // logo
				$customLogo                 = $( '#learts_custom_logo' ),
				customLogo                  = $customLogo.val(),
				$logo                       = $( '.cmb-row.cmb2-id-learts-logo' ),
				$logoAlt                    = $( '.cmb-row.cmb2-id-learts-logo-alt' ),
				$logoMobile                 = $( '.cmb-row.cmb2-id-learts-logo-mobile' ),
				$logoMobileAlt              = $( '.cmb-row.cmb2-id-learts-logo-mobile-alt' ),

				// Off-Canvas Sidebar
				$offCanvasButtonOn          = $( '#learts_offcanvas_button_on' ),
				$offCanvasAction            = $( '#learts_offcanvas_action' ),
				$offCanvasPosition          = $( '.cmb-row.cmb2-id-learts-offcanvas-position' ),
				$offCanvasCustom            = $( '.cmb-row.cmb2-id-learts-offcanvas-custom-sidebar' ),
				$offCanvasActionRow         = $( '.cmb-row.cmb2-id-learts-offcanvas-action' ),
				$offCanvasCorlor            = $( '.cmb-row.cmb2-id-learts-offcanvas-button-color' ),
				offCanvasActionVal          = $offCanvasAction.val(),
				offCanvasSideBarVal         = $offCanvasButtonOn.val(),

				// Page Title
				$pageTitleOn                = $( '#learts_page_title_on' ),
				$pageTitleStyle             = $( '#learts_page_title_style' ),
				$disableParallaxRow         = $( '.cmb-row.cmb2-id-learts-disable-parallax' ),
				$removeWhiteSpaceRow        = $( '.cmb-row.cmb2-id-learts-remove-whitespace' ),
				$pageTitleStyleRow          = $( '.cmb-row.cmb2-id-learts-page-title-style' ),
				$pageTitleTextColorRow      = $( '.cmb-row.cmb2-id-learts-page-title-text-color' ),
				$pageSubTitleColorRow       = $( '.cmb-row.cmb2-id-learts-page-subtitle-color' ),
				$pageTitleBgColorRow        = $( '.cmb-row.cmb2-id-learts-page-title-bg-color' ),
				$pageTitleOverlayColorRow   = $( '.cmb-row.cmb2-id-learts-page-title-overlay-color' ),
				$pageTitleBgImageRow        = $( '.cmb-row.cmb2-id-learts-page-title-bg-image' ),
				pageTitleOnVal              = $pageTitleOn.val(),
				pageTitleStyleVal           = $pageTitleStyle.val(),

				// Breadcrumbs
				$breadCrumbsOn              = $( '#learts_breadcrumbs' ),
				$breadCrumbsPosition        = $( '.cmb-row.cmb2-id-learts-breadcrumbs-position' ),
				breadCrumbsOnVal            = $breadCrumbsOn.val(),

				// Page Sidebar
				$pageSideBarPosition        = $( '#learts_page_sidebar_config' ),
				$customPageSideBar          = $( '.cmb-row.cmb2-id-learts-page-custom-sidebar' ),
				pageSideBarPositionVal      = $pageSideBarPosition.val(),

				// Post Sidebar
				$postSideBarPosition        = $( '#learts_post_sidebar_config' ),
				$customPostSideBar          = $( '.cmb-row.cmb2-id-learts-post-custom-sidebar' ),
				postSideBarPositionVal      = $postSideBarPosition.val(),

				// Product general
				$productShowFeaturedImage   = $( '#learts_show_featured_images' ),
				productShowFeaturedImageVal = $productShowFeaturedImage.val(),
				$thumbnailPosition          = $( '.cmb-row.cmb2-id-learts-product-thumbnails-position' ),
				$productPageLayout          = $( '#learts_product_page_layout' ),
				productPageLayout           = $productPageLayout.val(),
				$productBackground          = $( '.cmb-row.cmb2-id-learts-product-bgcolor' ),

				// Product Sidebar
				$productSideBarPosition     = $( '#learts_product_sidebar_config' ),
				$customProductSideBar       = $( '.cmb-row.cmb2-id-learts-product-custom-sidebar' ),
			...
			return {
				...
				events: function() {
					...
					$customLogo.on( 'change', function() {
						leartsAdmin.toggleLogoSettings( $( this ).val() == 'on' );
					} );

					$offCanvasButtonOn.on( 'change', function() {
						leartsAdmin.toggleOffCanvasSettings( $( this ).val() == 'off' );
					} );

					$offCanvasAction.on( 'change', function() {
						leartsAdmin.toggleOffCanvasAction( $( this ).val() );
					} );

					$pageTitleOn.on( 'change', function() {
						leartsAdmin.togglePageTitleSettings( $( this ).val() == 'off' );
					} );

					$pageTitleStyle.on( 'change', function() {
						leartsAdmin.togglePageTitleStyle( $( this ).val() );
					} );

					$breadCrumbsOn.on( 'change', function() {
						leartsAdmin.toggleBreadCrumbsSettings( $( this ).val() == 'off' );
					} );

					$pageSideBarPosition.on( 'change', function() {
						leartsAdmin.togglePageSidebarPosition( $( this ).val() );
					} );

					$postSideBarPosition.on( 'change', function() {
						leartsAdmin.togglePostSidebarPosition( $( this ).val() );
					} );

					$productSideBarPosition.on( 'change', function() {
						leartsAdmin.toggleProductSidebarPosition( $( this ).val() );
					} );

					$productPageLayout.on( 'change', function() {
						leartsAdmin.toggleProductPageLayout( $( this ).val() );
					} );

					$productShowFeaturedImage.on( 'change', function() {
						leartsAdmin.toggleFeaturedImages( $( this ).val() == 'off' || $( this ).val() == 'default' );
					} );
				},
				...
				togglePageTitleSettings: function( off ) {
					...
					if ( off ) {
						...
						leartsAdmin.togglePageTitleStyle( pageTitleStyle );
					}
...
var leartsProductAttributeFilterDependencyCallback = function() {
	(
		function( $, that ) {
			var $filterDropdown,
				$empty;
