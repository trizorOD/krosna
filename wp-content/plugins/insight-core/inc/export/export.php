...
class InsightCore_Export {
	...
	function export_sidebars() {
		...
		$sidebars = json_encode( get_option( 'insight_core_sidebars', '' ) );
	...
	function export_tables(
		...
		$backup_name = $backup_name ? $backup_name : $name . "___(" . date( 'H-i-s' ) . "_" . date( 'd-m-Y' ) . ")__rand" . rand( 1,
				11111111 ) . ".sql";
		ob_get_clean();
	...
	function export_media_packages() {
		...
		$dst = ABSPATH . '/wp-content' . DS . INSIGHT_CORE_THEME_SLUG . '-' . $_POST['demo'];
