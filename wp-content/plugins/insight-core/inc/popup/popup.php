...
if ( ! class_exists( 'Insight_Popup' ) ) {
	...
	class Insight_Popup {
		...
		function insight_popup_frontend_scripts() {
			...
			wp_enqueue_style( 'animate-css', plugins_url( 'assets/css/animate.min.css', __FILE__ ) );
		...
		function insight_popup_save( $popup_id, $post, $update ) {
			...
			if ( ! empty( $popups ) ) {
				...
				foreach ( $popups as $popup ) {
					...
					if ( $popup_id == $popup->ID && isset( $_POST['insight_popup_show_post_type'] ) ) {
						$post_type = $_POST['insight_popup_show_post_type'];
					}
