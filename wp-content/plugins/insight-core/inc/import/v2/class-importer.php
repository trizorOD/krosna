...
class InsightCore_Importer_2 extends WP_Importer {
	...
	function check_writeable() {
		...
		if ( ! is_writable( ABSPATH . 'wp-content' ) ) {
			$passed = false;
			echo '<script type="text/javascript">' . 'text_status(\'\');' . 'is_error(\'' . sprintf( wp_kses( __( 'Could not write files into directory: <strong>%swp-content</strong>',
					'insight-core' ),
					array(
						'strong' => array(),
					) ),
			...
			echo '<script type="text/javascript">>document.getElementById("errorImportMsg").innerHTML += \'<div class="notice notice-success">' . sprintf( wp_kses( __( 'Writable Passed: <strong>%s</strong>wp-content',
					'insight-core' ),
					array(
						'strong' => array(),
					) ),
					str_replace( '\\',
						'/',
						ABSPATH ) ) . '</div>\';document.getElementById("errorImportMsg").style.display = "inline-block";</script>';
		}

		echo '<br /><form action="" method="post" class="retry-form">' . '<em>' . esc_html__( 'Tips: Try to change Chmod of folder to 755 or 777',
				'insight-core' ) . '</em><br/><br/>' . '<input type="submit" id="submitbtn" class="button button-primary" value="' . esc_html__( 'Click to Retry',
				'insight-core' ) . '" />' . '<input type="hidden" value="1" name="import_sample_data">' . '<input type="hidden" value="' . esc_attr( $this->demo ) . '" name="demo"/>' . '</form>';
	...
	function download_package( $_tmppath ) {
		...
		if ( ! is_dir( $_tmppath ) ) {
			...
			$demos = $this->options['import_full_demo'] ? apply_filters( 'insight_core_import_demos',
				array() ) : apply_filters( 'insight_core_import_dummies', array() );
			...
			if ( $this->options['import_full_demo'] ) {
				echo '<script type="text/javascript">progress_status("dl");text_status(\'' . esc_html__( 'Downloading package...',
						'insight-core' ) . '\');</script>';
			}
			...
			if ( ! is_wp_error( $package ) ) {
					...
					if ( @mkdir( $_tmppath, 0755 ) ) {
						...
						if ( is_wp_error( $unzip ) ) {
							echo '<script type="text/javascript">is_error(\'' .
							     ...
							     sprintf( wp_kses( __( '<strong>ERROR %s:</strong> Could not extract demo media package. Please contact our support staff.',
								     'insight-core' ),
								     array(
									     'strong' => array(),
									     'a'      => array(
				...
				if ( $this->options['import_full_demo'] ) {
					echo '<script type="text/javascript">is_error(\'' .
					     ...
					     sprintf( wp_kses( __( '<strong>ERROR %s:</strong> Could not download demo media package. Please use <a href="%s" target="_blank">this direct link</a> or contact our support staff.',
						     'insight-core' ),
						     array(
							     'strong' => array(),
							     'a'      => array(
		...
		if ( is_dir( $_tmppath ) ) {
			...
			foreach ( $_new as $key => $value ) {
				...
				if ( $value == 4 ) {
					...
					@copy( $_tmppath . DS . urldecode( $key ), $_cpath . 'uploads' . DS . urldecode( $key ) );
			...
			echo '<script type="text/javascript">is_error(\'' .
			     ...
			     sprintf( wp_kses( __( '<strong>ERROR %s:</strong> Could not found temporary folder. Please contact our support staff.',
				     'insight-core' ),
				     array(
					     'strong' => array(),
					     'a'      => array(
	...
	function get_data( $type, $unserialize = true ) {
		...
		$file = get_template_directory() . DS . 'assets' . DS . 'import' . DS . $this->demo . DS . $type . '.txt';
	...
	function read_widget_logic_file() {
		// Widget Logic
		$widget_logic_file = get_template_directory() . '/assets/import/' . $this->demo . '/widget_logic_options.txt';
	...
	function import_sidebars() {
		...
		if ( class_exists( 'InsightCore_Sidebars' ) ) {
			...
			if ( $this->options['import_full_demo'] ) {
				echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Sidebars</b>',
						'insight-core' ),
						array( 'b' => array() ) ) . '\' );</script>';
			}

			if ( $data ) {
				update_option( 'insight_core_sidebars', (array) $data );

				$sidebars = new InsightCore_Sidebars();
				$sidebars->widgets_init();
			}
		}
	}
	...
	function import_widgets() {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			echo '<script type="text/javascript">is_error(\'' .
			     ...
			     sprintf( wp_kses( __( '<strong>ERROR %s:</strong> Could not read the widget sample data file. Please contact our support staff.',
				     'insight-core' ),
				     array(
					     'strong' => array(),
					     'a'      => array(
		...
		if ( $this->options['import_full_demo'] ) {
			echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Widgets</b>' ),
					array( 'b' => array() ) ) . '\' );</script>';
		}
	...
	function import_menus() {
		...
		if ( $this->options['import_full_demo'] ) {
			echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Menus</b>' ),
					array( 'b' => array() ) ) . '\' );</script>';
		}
		...
		if ( ! empty( $menu_data ) ) {
			...
			foreach ( $menu_data as $registered_menu => $menu_slug ) {
				...
				$term_rows = $wpdb->get_results( "SELECT * FROM $insight_core_terms_table where slug='{$menu_slug}'",
					ARRAY_A );
	...
	function import_customizer_options() {
		...
		if ( $this->options['import_full_demo'] ) {
			echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Customizer Options</b>',
					'insight-core' ),
					array( 'b' => array() ) ) . '\' );</script>';
		}
	...
	function import_woocommerce_attributes() {
		...
		$_x_path = get_template_directory() . DS . 'assets' . DS . 'import' . DS . $this->demo . DS;
					...
					foreach ( $parsed['terms'] as $term ) {
						...
						if ( strstr( $term['taxonomy'], 'pa_' ) ) {
							if ( ! taxonomy_exists( $term['taxonomy'] ) ) {
								$attribute_name = wc_sanitize_taxonomy_name( str_replace( 'pa_',
									'',
									$term['taxonomy'] ) );
								...
								register_taxonomy( $term['taxonomy'],
									apply_filters( 'woocommerce_taxonomy_objects_' . $term['taxonomy'],
										array( 'product' ) ),
									apply_filters( 'woocommerce_taxonomy_args_' . $term['taxonomy'],
										array(
											'hierarchical' => true,
											'show_ui'      => false,
											'query_var'    => true,
	...
	function import_woocommerce_pages() {
		...
		if ( $this->options['import_full_demo'] ) {
			echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>WooCommerce</b>' ),
					array( 'b' => array() ) ) . '\' );</script>';
		}
	...
	function import_attribute_swatches() {
		...
		if ( $this->options['import_full_demo'] ) {
			echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Attribute Swatches</b>',
					'insight-core' ),
					array( 'b' => array() ) ) . '\' );</script>';
		}
	...
	function import_rev_sliders() {
		...
		if ( $this->options['import_full_demo'] ) {
			echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Revolution Slider</b>' ),
					array( 'b' => array() ) ) . '\' );</script>';
		}
		...
		if ( get_option( 'insight_core_import_revsliders' ) == false && $this->options['import_full_demo'] ) {
			...
			$rev_files = glob( get_template_directory() . '/assets/import/' . $this->demo . '/rev_sliders/*.zip' );
	...
	function import_essential_grid() {
		...
		$file = get_template_directory() . '/assets/import/' . $this->demo . '/essential_grid.txt';
		...
		try {
			...
			if ( $this->options['import_full_demo'] ) {
				echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Essential Grid</b>' ),
						array( 'b' => array() ) ) . '\' );</script>';
			}
			...
			if ( ! empty( $navigation_skins ) && is_array( $navigation_skins ) ) {
				if ( ! empty( $navigation_skins ) ) {
					$navigation_skins_imported = $im->import_navigation_skins( @$navigation_skins,
						$export_navigation_skins );
				}
			}
			...
			echo '<script type="text/javascript">is_error(\'' .
			     ...
			     sprintf( wp_kses( __( '<strong>ERROR %s:</strong> Could not download demo media package. Please contact our support staff.',
				     'insight-core' ),
				     array(
					     'strong' => array(),
					     'a'      => array(
	...
	function fix_essential_grid() {
		...
		if ( $this->options['import_full_demo'] ) {
			echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Fix <b>Essential Grid</b> Filter' ),
					array( 'b' => array() ) ) . '\' );</script>';
		}
		...
		foreach ( $myrows as $myrow ) {
			...
			if ( isset( $postparams->post_category ) && ( $postparams->post_category != "" ) ) {
				...
				$wpdb->update( $table_prefix . 'eg_grids',
					array(
	...
	public function dispatch() {
		...
		if ( $this->options['import_full_demo'] ) {
			echo '<script type="text/javascript">progress_status(0);text_status( \'' . esc_html__( 'Preparing for add media...',
					'insight-core' ) . '\' );</script>';
		}
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse',
				__( 'Could not open the file for parsing', 'insight-core' ) );
		}
			...
			switch ( $reader->name ) {
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) && WP_DEBUG ) {
						$this->logger->warning( sprintf( __( 'This WXR file (version %s) is newer than the importer (version %s) and may not be supported. Please consider updating.',
							'insight-core' ),
							$this->version,
	...
	public function import( $file ) {
		...
		while ( $reader->read() ) {
			...
			switch ( $reader->name ) {
				...
				case 'item':
					...
					if ( $this->options['import_full_demo'] ) {
						echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Posts</b>' ),
								array( 'b' => array() ) ) . '\' );</script>';
						@ob_flush();
						@flush();
					}
				...
				case 'wp:author':
					...
					if ( $this->options['import_full_demo'] ) {
						echo '<script type="text/javascript">text_status( \'' . esc_html__( 'Get author mapping',
								'insight-core' ) . '\' );</script>';
						@ob_flush();
						@flush();
					}
				...
				case 'wp:category':
					...
					if ( $this->options['import_full_demo'] ) {
						echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Categories</b>' ),
								array( 'b' => array() ) ) . '\' );</script>';
						@ob_flush();
						@flush();
					}
				...
				case 'wp:tag':
					...
					if ( $this->options['import_full_demo'] ) {
						echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Tags</b>' ),
								array( 'b' => array() ) ) . '\' );</script>';
						@ob_flush();
						@flush();
					}
				...
				case 'wp:term':
					...
					if ( $this->options['import_full_demo'] ) {
						echo '<script type="text/javascript">text_status( \'' . wp_kses( __( 'Adding <b>Terms</b>' ),
								array( 'b' => array() ) ) . '\' );</script>';
						@ob_flush();
						@flush();
					}
		...
		if ( $this->options['import_full_demo'] ) {
			echo '<script type="text/javascript">text_status( \'' . esc_html__( 'Updating incorrect/missing infomation in the database...',
					'insight-core' ) . '\');</script>';
			@ob_flush();
			@flush();
		}
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'wxr_importer.file_missing',
				__( 'The file does not exist, please try again.', 'insight-core' ) );
		}
			...
			switch ( $child->tagName ) {
				...
				case 'wp:status':
					...
					if ( $data['post_status'] === 'auto-draft' ) {
						// Bail now
						return new WP_Error( 'wxr_importer.post.cannot_import_draft',
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! $post_type_object ) {
			if ( WP_DEBUG ) {
				$this->logger->warning( sprintf( __( 'Failed to import "%s": Invalid post type %s', 'insight-core' ),
					$data['post_title'],
					$data['post_type'] ) );
			}
		...
		if ( $post_exists ) {
			if ( WP_DEBUG ) {
				$this->logger->info( sprintf( __( '%s "%s" already exists.', 'insight-core' ),
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				if ( WP_DEBUG ) {
					$this->logger->notice( sprintf( __( 'Skipping attachment "%s", fetching attachments disabled' ),
						$data['post_title'] ) );
				}
			...
			if ( $this->options['import_full_demo'] ) {
				echo '<script type="text/javascript">text_status( \'' . sprintf( wp_kses( __( 'Adding Page <b>%s</b>',
						'insight-core' ),
						array(
							'b' => array(),
						) ),
			...
			if ( $this->options['import_full_demo'] ) {
				echo '<script type="text/javascript">text_status( \'' . sprintf( wp_kses( __( 'Adding Post <b>%s</b>',
						'insight-core' ),
						array(
							'b' => array(),
						) ),
			...
			if ( $this->options['import_full_demo'] ) {
				echo '<script type="text/javascript">text_status( \'' . sprintf( wp_kses( __( 'Adding Custom Post Type <b>%s</b>',
						'insight-core' ),
						array(
							'b' => array(),
						) ),
		...
		if ( is_wp_error( $post_id ) ) {
			if ( WP_DEBUG ) {
				$this->logger->error( sprintf( __( 'Failed to import "%s" (%s)', 'insight-core' ),
					$data['post_title'],
					$post_type_object->labels->singular_name ) );
		...
		if ( WP_DEBUG ) {
			$this->logger->info( sprintf( __( 'Imported "%s" (%s)', 'insight-core' ),
				$data['post_title'],
				$post_type_object->labels->singular_name ) );
			$this->logger->debug( sprintf( __( 'Post %d remapped to %d', 'insight-core' ),
				$original_id,
				$post_id ) );
		}
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! empty( $guidraw[1] ) ) {
			$guidraw = INSIGHT_CORE_SITE_URI . DS . 'wp-content' . $guidraw[1];
		} else {
			$guidraw = INSIGHT_CORE_SITE_URI . DS . 'wp-content' . $guidraw[0];
		}

		if ( $this->options['import_full_demo'] ) {
			echo '<script type="text/javascript">' . 'progress_status( ' . ( $this->imageCount / $this->totalImages ) . ' );' . 'text_status( \'' . sprintf( __( 'Adding Media %s',
					'insight-core' ),
					$guidraw ) . '\' );</script>';
			@ob_flush();
			@flush();
		...
		if ( file_exists( $_urlxc ) ) {
			$upload = array(
				'file' => $_urlxc,
				'url'  => INSIGHT_CORE_SITE_URI . '/wp-content' . $_urlxz[1],
			);
	...
	protected function process_author( $data, $meta ) {
		...
		if ( is_wp_error( $user_id ) ) {
			...
			if ( ! $user_id->errors['existing_user_login'] ) {
				if ( WP_DEBUG ) {
					$this->logger->error( sprintf( __( 'Failed to import user "%s"', 'insight-core' ),
						$userdata['user_login'] ) );
		...
		if ( WP_DEBUG ) {
			$this->logger->info( sprintf( __( 'Imported user "%s"', 'insight-core' ),
				$userdata['user_login'] ) );
			$this->logger->debug( sprintf( __( 'User %d remapped to %d', 'insight-core' ),
				$original_id,
				$user_id ) );
		}
	...
	protected function process_term( $data, $meta ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			if ( WP_DEBUG ) {
				$this->logger->warning( sprintf( __( 'Failed to import %s %s', 'insight-core' ),
					$data['taxonomy'],
					$data['name'] ) );
		...
		if ( ! empty( $meta ) ) {
			foreach ( $meta as $meta_item ) {
				...
				if ( is_wp_error( $result ) ) {
					if ( WP_DEBUG ) {
						$this->logger->warning( sprintf( __( 'Failed to add metakey: %s, metavalue: %s to term_id: %d',
							'wordpress-importer' ),
							$meta_item['key'],
							$meta_item['value'],
							$term_id ) );
					...
					if ( WP_DEBUG ) {
						$this->logger->debug( sprintf( __( 'Meta for term_id %d : %s => %s ; successfully added!',
							'wordpress-importer' ),
							$term_id,
							$meta_item['key'],
							$meta_item['value'] ) );
		...
		if ( WP_DEBUG ) {
			$this->logger->info( sprintf( __( 'Imported "%s" (%s)', 'insight-core' ),
				$data['name'],
				$data['taxonomy'] ) );
			$this->logger->debug( sprintf( __( 'Term %d remapped to %d', 'insight-core' ),
				$original_id,
				$term_id ) );
		}
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error( 'import_file_error',
				sprintf( __( 'Remote server returned %1$d %2$s for %3$s', 'insight-core' ),
					$code,
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			@unlink( $upload['file'] );
			$message = sprintf( __( 'Remote file is too large, limit is %s', 'insight-core' ),
	...
	protected function post_process_posts( $todo ) {
		foreach ( $todo as $post_id => $_ ) {
			...
			if ( WP_DEBUG ) {
				$this->logger->debug( sprintf( // Note: title intentionally not used to skip extra processing
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					if ( WP_DEBUG ) {
						$this->logger->warning( sprintf( __( 'Could not find the post parent for "%s" (post #%d)',
							'insight-core' ),
							get_the_title( $post_id ),
							$post_id ) );
						$this->logger->debug( sprintf( __( 'Post %d was imported with parent %d, but could not be found',
							'insight-core' ),
							$post_id,
							$parent_id ) );
					}
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					if ( WP_DEBUG ) {
						$this->logger->warning( sprintf( __( 'Could not find the author for "%s" (post #%d)',
							'insight-core' ),
							get_the_title( $post_id ),
							$post_id ) );
						$this->logger->debug( sprintf( __( 'Post %d was imported with author "%s", but could not be found',
							'insight-core' ),
							$post_id,
							$author_slug ) );
					}
			...
			if ( empty( $data ) ) {
				if ( WP_DEBUG ) {
					$this->logger->debug( sprintf( __( 'Post %d was marked for post-processing, but none was required.',
						'insight-core' ),
						$post_id ) );
				}
				continue;
			...
			if ( is_wp_error( $result ) ) {
				if ( WP_DEBUG ) {
					$this->logger->warning( sprintf( __( 'Could not update "%s" (post #%d) with mapped data',
						'insight-core' ),
						get_the_title( $post_id ),
						$post_id ) );
					$this->logger->debug( $result->get_error_message() );
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			if ( WP_DEBUG ) {
				$this->logger->warning( sprintf( __( 'Could not find the menu object for "%s" (post #%d)',
					'insight-core' ),
					get_the_title( $post_id ),
					$post_id ) );
				$this->logger->debug( sprintf( __( 'Post %d was imported with object "%d" of type "%s", but could not be found',
					'insight-core' ),
					$post_id,
					$menu_object_id,
					$menu_item_type ) );
	...
	protected function post_process_terms( $terms_to_be_remapped ) {
		...
		foreach ( $terms_to_be_remapped as $termid => $term_taxonomy ) {
			...
			if ( empty( $termid ) or ! ( is_numeric( $termid ) ) ) {
				if ( WP_DEBUG ) {
					$this->logger->warning( sprintf( __( 'Faulty term_id provided in terms-to-be-remapped array %s',
						'wordpress-importer' ),
						$termid ) );
				}
				continue;
			...
			if ( empty( $term_taxonomy ) ) {
				if ( WP_DEBUG ) {
					$this->logger->warning( sprintf( __( 'No taxonomy provided in terms-to-be-remapped array for term #%d',
						'wordpress-importer' ),
						$term_id ) );
				}
				continue;
			...
			if ( empty( $parent_slug ) ) {
				if ( WP_DEBUG ) {
					$this->logger->warning( sprintf( __( 'No parent_slug identified in remapping-array for term: %d',
						'wordpress-importer' ),
						$term_id ) );
				}
				continue;
			...
			if ( ! isset( $this->mapping['term_slug'][ $parent_slug ] ) or ! is_numeric( $this->mapping['term_slug'][ $parent_slug ] ) ) {
				if ( WP_DEBUG ) {
					$this->logger->warning( sprintf( __( 'The term(%d)"s parent_slug (%s) is not found in the remapping-array.',
						'wordpress-importer' ),
						$term_id,
						$parent_slug ) );
				}
			...
			if ( empty( $termattributes ) ) {
				if ( WP_DEBUG ) {
					$this->logger->warning( sprintf( __( 'No data returned by get_term_by for term_id #%d',
						'wordpress-importer' ),
						$term_id ) );
				}
				continue;
			...
			if ( is_wp_error( $result ) ) {
				if ( WP_DEBUG ) {
					$this->logger->warning( sprintf( __( 'Could not update "%s" (term #%d) with mapped data',
						'wordpress-importer' ),
						$termattributes['name'],
						$term_id ) );
					$this->logger->debug( $result->get_error_message() );
			...
			if ( WP_DEBUG ) {
				$this->logger->debug( sprintf( __( 'Term %d was successfully updated with parent %d',
					'wordpress-importer' ),
					$term_id,
					$mapped_parent ) );
			}
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					if ( WP_DEBUG ) {
						$this->logger->warning( sprintf( __( 'Could not find the comment parent for comment #%d',
							'insight-core' ),
							$comment_id ) );
						$this->logger->debug( sprintf( __( 'Comment %d was imported with parent %d, but could not be found',
							'insight-core' ),
							$comment_id,
							$parent_id ) );
					}
			...
			if ( ! empty( $author_id ) && WP_DEBUG ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					if ( WP_DEBUG ) {
						$this->logger->warning( sprintf( __( 'Could not find the author for comment #%d',
							'insight-core' ),
							$comment_id ) );
						$this->logger->debug( sprintf( __( 'Comment %d was imported with author %d, but could not be found',
							'insight-core' ),
							$comment_id,
							$author_id ) );
					}
			...
			if ( empty( $result ) ) {
				if ( WP_DEBUG ) {
					$this->logger->warning( sprintf( __( 'Could not update comment #%d with mapped data',
						'insight-core' ),
						$comment_id ) );
				}
				continue;
	...
	protected function replace_attachment_urls_in_content() {
		...
		foreach ( $this->url_remap as $from_url => $to_url ) {
			...
			$query = $wpdb->prepare( "UPDATE {$wpdb->posts} SET post_content = REPLACE(post_content, %s, %s)",
				$from_url,
				$to_url );
			$wpdb->query( $query );

			// remap enclosure urls
			$query  = $wpdb->prepare( "UPDATE {$wpdb->postmeta} SET meta_value = REPLACE(meta_value, %s, %s) WHERE meta_key='enclosure'",
				$from_url,
				$to_url );
