...
$new_version_available = isset( $last_update->response[ INSIGHT_CORE_THEME_SLUG ] ) && ( version_compare( $last_update->response[ INSIGHT_CORE_THEME_SLUG ]['new_version'],
			INSIGHT_CORE_THEME_VERSION ) == 1 );
				...
				<div class="box-body">
					...
					if ( InsightCore::check_valid_update() ) {
						...
						printf( __( 'There is a new version of %1$s available. <a href="%2$s" %3$s>View version %4$s details</a> or <a href="%5$s" %6$s>update now</a>.',
							...
							$last_update->response[ INSIGHT_CORE_THEME_SLUG ]['new_version'],
							wp_nonce_url( self_admin_url( 'update.php?action=upgrade-theme&theme=' ) . INSIGHT_CORE_THEME_SLUG,
								'upgrade-theme_' . INSIGHT_CORE_THEME_SLUG ),
