 ...
 *
 * Shortcode class
 * @var $this WPBakeryShortCode_Learts_Blog
 */
global $learts_options;

$learts_options['archive_display_type'] = 'standard';
...
$col_lg = 'col-lg-' . Learts_VC::calculate_column_width( $columns );

$shortcode_post_class = 'col-md-6 ' . $col_lg . ' post post-item';

$css_id = Learts_VC::get_learts_shortcode_id( 'learts-blog' );
...
<div id="<?php echo esc_attr( $css_id ); ?>"
	...
	<div class="post-grid-layout <?php echo esc_attr( $grid_align ); ?> posts row">
		...
		<div class="post-masonry-layout <?php echo esc_attr( $grid_align ); ?> posts row">
			...
			<?php if ( $this->num_pages >= 2 && $pagination_type == 'default' && $view != 'carousel' && $pagination_type != 'none' ) { ?>
			...
			<?php
			if ( $this->num_pages >= 2 && $pagination_type != 'default' && $view != 'carousel' && $pagination_type != 'none' ) {

				$load_more_atts = array(
					'container'      => '#' . $css_id,
					'post_type'      => 'post',
					'paged'          => 1,
					'posts_per_page' => apply_filters( 'learts_ajax_posts_per_page', $posts_per_page ),
					'view'           => $view,
					'columns'        => $columns,
					'orderby'        => $orderby,
					'order'          => $order,
