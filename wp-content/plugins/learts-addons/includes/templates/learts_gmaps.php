 ...
 * @var $css
 * Shortcode class
 * @var $this WPBakeryShortCode_Learts_Gmaps
 */
...
wp_enqueue_script( 'google-maps',
	'https://maps.google.com/maps/api/js?key=' . Learts_Addons::get_option( 'gmaps_api_key' ) . '&amp;language=en' );
wp_enqueue_script( 'gmap3', LEARTS_LIBS_URI . '/gmap3/js/gmap3.min.js', array( 'jquery' ), null, true );
			...
			function() {
				...
				if ( gmDiv.length ) {
					...
					markers.push( {
						...
						<?php if (! isset( $new_marker['icon'] ) || $new_marker['icon'] == '') { ?>icon: '<?php echo LEARTS_THEME_URI ?>/assets/images/map_marker.png',
					...
					gmDiv.gmap3( {
						...
						echo esc_attr( $map_style == 'default' ) ? 'google.maps.MapTypeId.' . strtoupper( $map_type ) : '\'custom\'';
					} else {
