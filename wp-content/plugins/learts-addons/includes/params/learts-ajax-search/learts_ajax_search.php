...
array(
  'type'       => 'ajax-search',
  'heading'    => esc_html__( 'Categories', 'learts' ),
  'param_name' => 'ajax',
	'options'     => array(
...
*/
if ( ! class_exists( 'Learts_Ajax_Search' ) ) {
	...
	class Learts_Ajax_Search {
			...
			if ( $type == 'post_type' ) {
				...
				if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
					foreach ( $post_meta_infos as $value ) {
						...
						$data['name'] = esc_html__( 'Id', 'learts' ) . ': ' . $value['id'] . ( ( strlen( $value['name'] ) > 0 ) ? ' - ' . esc_html__( 'Name', 'learts' ) . ': ' . $value['name'] : '' ) . ( ( strlen( $value['slug'] ) > 0 ) ? ' - ' . esc_html__( 'Slug', 'learts' ) . ': ' . $value['slug'] : '' );
						$result[]     = $data;
					}
				}
			}
		...
		public function admin_scripts() {
			wp_enqueue_style( 'learts-ajax-search', LEARTS_ADDONS_URL . 'includes/params/learts-ajax-search/token-input.css' );
			wp_enqueue_script( 'learts-ajax-search', LEARTS_ADDONS_URL . 'includes/params/learts-ajax-search/jquery.tokeninput.min.js', array( 'jquery' ), null, true );
		}
			...
			if ( $value != '' ) {
				...
				if ( $type == 'post_type' ) {
					...
					foreach ( $value_items as $value_item ) {
						...
						$pre_populate .= '{id: "' . trim( $value_item ) . '", name: "ID: ' . $value_item->term_id . ( ( strlen( $value_item_info->name ) > 0 ) ? ' - ' . esc_html__( 'Name', 'learts' ) . ': ' . $value_item_info->name : '' ) . ( strlen( $value_item_info->slug ) > 0 ? ' - ' . esc_html__( 'Slug', 'learts' ) . ': ' . $value_item_info->slug : '' ) . '"},';
					}
				}
			}
