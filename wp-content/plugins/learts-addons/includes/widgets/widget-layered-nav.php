 ...
 *
 */
if ( ! class_exists( 'learts_Layered_Nav_Widget' ) )  {
	add_action( 'widgets_init', 'load_learts_layered_nav_widget' );

	function load_learts_layered_nav_widget() {
		register_widget( 'learts_Layered_Nav_Widget' );
	}

	/**
	 * Layered Nav Widget by ThemeMove
	 */
	class learts_Layered_Nav_Widget extends WPH_Widget {
		...
		function __construct() {
			...
			$args = array(
				...
				'label'       => '&#x1f3c1; &nbsp;' . esc_html__( 'Learts WooCommerce Layered Nav',
						'learts' ),
			...
			$args['fields'] = array(
				...
				array(
					...
					'type'   => 'text',
					'class'  => 'widefat',
					'std'    => esc_html__( 'Filter by', 'learts' ),
					'filter' => 'strip_tags|esc_attr',
				),

				array(
					'name'    => esc_html__( 'Attribute', 'learts' ),
					'id'      => 'attribute',
					'type'    => 'select',
					'std'     => '',
				...
				array(
					...
					'type'    => 'select',
					'std'     => 'and',
					'name'    => esc_html__( 'Query type', 'learts' ),
					'options' => array(
						esc_html__( 'AND', 'learts' ) => 'and',
						esc_html__( 'OR', 'learts' )  => 'or',
					),
				),
				...
				array(
					...
					'type'    => 'select',
					'std'     => 'list',
					'name'    => esc_html__( 'Display type', 'learts' ),
					'options' => array(
						esc_html__( 'List', 'learts' )     => 'list',
						esc_html__( 'Inline', 'learts' )   => 'inline',
						esc_html__( 'Dropdown', 'learts' ) => 'dropdown',
					),
				),
				...
				array(
					...
					'type'    => 'select',
					'std'     => 'on',
					'name'    => esc_html__( 'Show labels', 'learts' ),
					'options' => array(
						esc_html__( 'ON', 'learts' )  => 'on',
						esc_html__( 'OFF', 'learts' ) => 'off',
					),
				),
				...
				array(
					...
					'type'    => 'select',
					'std'     => 'on',
					'name'    => esc_html__( 'Show items count', 'learts' ),
					'options' => array(
						esc_html__( 'ON', 'learts' )  => 'on',
						esc_html__( 'OFF', 'learts' ) => 'off',
					),
		...
		protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
			...
			if ( $taxonomy !== $this->get_current_taxonomy() ) {
				...
				$any_label            = apply_filters( 'woocommerce_layered_nav_any_label',
					sprintf( __( 'Any %s', 'learts' ), $taxonomy_label ),
					$taxonomy_label,
					$taxonomy );
				...
				echo '<select class="dropdown_layered_nav_' . $taxonomy_filter_name . '" data-filter-url="' . preg_replace( '%\/page\/[0-9]+%',
						...
						str_replace( array(
							...
							esc_js( add_query_arg( 'filtering',
								...
								) ) ) ) ) ) . "&filter_" . esc_js( $taxonomy_filter_name ) . "=learts_FILTER_VALUE" . '">';
