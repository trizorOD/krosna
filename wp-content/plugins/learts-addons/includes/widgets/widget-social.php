...
/**
 Social Widget
 */
if ( ! class_exists( 'Learts_Social_Widget' ) ) {

	add_action( 'widgets_init', 'load_learts_widget' );

	function load_learts_widget() {
		register_widget( 'Learts_Social_Widget' );
	}
	 ...
	 * @property mixed data
	 */
	class learts_Social_Widget extends WP_Widget {
		...
		function __construct() {
			parent::__construct( 'tm_social',
				'&#x1f534; &nbsp;' . esc_html__( 'Learts Social', 'learts' ),
				array( 'description' => esc_html__( 'Display your social networks.','learts' ) ) );

			$this->social_networks = Learts_VC::social_icons( false );
		}
		...
		function widget( $args, $instance ) {
			...
			if ( $instance['source'] == 'default' ) {
				$output .= Learts_Templates::social_links();
			} else {
		...
		function form( $instance ) {
			...
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'source' ) ); ?>"><?php _e( 'Select Social Source',
						'learts' ); ?></label>
				<select class="widefat"
					...
					<option value="default" <?php selected( $instance['source'], 'default', true ); ?>>
						<?php esc_html_e( 'Default (from Theme Options >> Social)', 'learts' ) ?>
					</option>
					<option value="custom" <?php selected( $instance['source'], 'custom', true ); ?>>
						<?php esc_html_e( 'Custom', 'learts' ) ?>
					</option>
				</select>
			</p>
			...
			<div class="tm-social-links" data-social-links="true">
				<p>
					<label><?php _e( 'Social links', 'learts' ); ?></label>
				</p>
				<div class="tm-social-links-inner">
					<table class="tm-table tm-social-links-table widefat">
						<tr>
							<th><strong><?php esc_html_e( 'Social Network', 'learts' ); ?></strong></th>
							<th><strong><?php esc_html_e( 'Link', 'learts' ); ?></strong></th>
						</tr>
						<?php
						foreach ( $this->social_networks as $key => $social ) {
							?>
							<tr>
								<td class="tm-social tm-social--<?php echo esc_attr( $key ); ?>">
									<span><i
											class="fa fa-<?php echo esc_attr( $key ) ?>"></i><?php echo esc_attr($social); ?></span>
								</td>
								<td>
