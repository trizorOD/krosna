        ...
        <div id="taxonomy-<?php echo $taxonomy; ?>" class="categorydiv">
            ...
            <ul id="<?php echo $taxonomy; ?>-tabs" class="category-tabs">
                ...
                <li class="hide-if-no-js"><a href="#<?php echo $taxonomy; ?>-pop"
                                             tabindex="3"><?php _e('Most Used', 'woocommerce-brands'); ?></a></li>
            </ul>
...
// create two taxonomies, genres and writers for the post type "product"
   ...
   public static function init_taxonomy() {
        ...
        if ('' === $category_base) {
            $slug = get_option('pw_woocommerce_brands_base', '');
        }
    ...
    public function add_brands_fields()
    {
        ...
        <div class="">
            ...
            <p><?php _e('Set Brand External Url (if you set the url, when visitor click on a brand name, this url will be displayed instead of brand page )', 'woocommerce-brands'); ?>
                .</p>
        </div>
        <br/>
        <div class="">
        ...
        <div class="form-field">
            ...
            <div id="brands_thumbnail" style="float:left;margin-right:10px;"><img src="<?php echo $image; ?>"
                                                                                  width="60px" height="60px"/></div>
            <div style="line-height:60px;">
                <input type="hidden" id="brands_thumbnail_id" name="brands_thumbnail_id"/>
                <button type="button"
                        class="upload_image_button button"><?php _e('Upload/Add image', 'woocommerce-brands'); ?></button>
                <button type="button"
                        class="remove_image_button button"><?php _e('Remove image', 'woocommerce-brands'); ?></button>
            </div>
    ...
    public function edit_brands_fields($term, $taxonomy)
    {
        ...
        <tr class="">
            ...
            <td>
                ...
                <span class="description"><?php _e('Set Brand External Url (if you set the url, when visitor click on a brand name, this url will be displayed instead of brand page )', 'woocommerce-brands'); ?>
                    .</span>
            </td>
    ...
    public function save_brands_fields($term_id, $tt_id, $taxonomy)
    {
        ...
        if (isset($_POST['brands_thumbnail_id']))
            update_term_meta($term_id, 'thumbnail_id', absint($_POST['brands_thumbnail_id']));

        if (isset($_POST['url'])) {

            update_term_meta($term_id, 'url', $_POST['url']);
        }
    ...
    public function brands_column($columns, $column, $id)
    {
        ...
        if ($column == "featured") {
            $display_type = get_term_meta($id, 'featured', true);
            if ($display_type == "1")
                $columns .= 'yes';
            else
                $columns .= 'no';
        ...
        if ($column == "url")
		{
            $columns .= get_term_meta($id, 'url', true);
		}
		if ( 'handle' === $column ) {
