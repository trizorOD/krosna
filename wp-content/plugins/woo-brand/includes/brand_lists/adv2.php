...
$word= __('All', 'woocommerce-brands');
$dropdown = '<label for="pw_brand_category_filter" class="pw_brand_category_filter_checkbox pw-active-filter">' . $word . '<input name="pw_woob_category[]" id="pw_brand_category_filter_all"  type="checkbox" value="0" data-id="' . $rand_id . '" checked /></label>';
...
foreach ($terms as $term) {
    ...
    if ($count == 0 && !$option_is_set && $empty_brand==1)
        continue;
    ...
    */
    ...
    if ($pw_show_image == "yes") {
        ...
        $brand_img = ' <a href="' . ($url == "" ? get_term_link($term->slug, $get_terms) : $url) . '"><img src="' . $image . '"  alt="' . esc_html($term->name) . '" /></a>';
    }
