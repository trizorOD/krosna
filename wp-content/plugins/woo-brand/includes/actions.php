...
function pw_brand_fetch_brands()
{
    ...
    if (is_array($pw_woob_category) && $pw_woob_category[0] != 0 && $empty_brand != 0) {
    ...
    */
    ...
    foreach ($terms as $term) {
        ...
        if (is_array($pw_woob_category) && $pw_woob_category[0] != 0) {
    ...
    if (trim($output_html) == '') {
        $output_html = __('There are no Brands !', 'woocommerce-brands');
...
function pw_brand_fetch_products()
{
    ...
    if (trim($output_html) == '') {
        $output_html = __('There are no items !', 'woocommerce-brands');
        $ret .= $output_html . "
					</div><!-- wb-allview-cat-cnt=-->";
        //end all-brand-alphabet
        echo $ret;
