<?php
	...
	$dropdown_brand=wp_dropdown_categories(
		array(
			'show_option_all'    => __('All (Select)','woocommerce-brands').get_option( 'pw_woocommerce_brands_base' ).')',
	...
	foreach($matched_products as $id)
	{
		...
		if(version_check_wocoommerce_brand('3.0')){
			...
			$cat =wc_get_product_category_list($id);	
			$tag =wc_get_product_tag_list($id);	
		}
		else
		{
		...
		$output_html.='
			<div class="'.$pw_mobile_columns.' '.$pw_tablet_columns.' '.$pw_columns.'">
				...
				if (isset($pw_show_image) && ($pw_show_image=='yes'))	
					$output_html.='<a href="'.$permalink.'" >'.$img.'</a>';
				$output_html.='
